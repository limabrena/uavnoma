...
data_parameter_generate_channel = [
    (2, 10.0, -6.0, 2.0, 4.0, 35.0, 2, 15, 1), # Valid # number_user, axis x user position, axis y user position, axis x uav position, axis y uav position, uav mean height, path_loss, rician_factor, power_los
    (4, 2.0, 15.0, 4.0, 2.0, 20.0, 2.2, 18, 2), # Invalid
    (2, 10.0, 10.0, 2.0, 3.0, 40.0, 2, 16, 2),  # Valid
    (4, -8.0, 15.0, 2.0, 3.0, 40.0, 8, 3, 4),   # Invalid
]
...
def test_channel_gain(number_user, x_u, y_u, x_r, y_r, uav_height_mean, path_loss, rician_factor, power_los):
    ...
    sigma = power_los / np.sqrt(2 * (rician_factor + 1))
    assert s > 0 and s < 2  # Non-negative
    assert sigma > 0 and sigma <= 1.5  # Non-negative    
